...
do
	java -jar VivoIngest.jar -action remove -removeFileName ${DELTA} -dbString ${DB_STRING} -localModelName ${MODEL} -password ${PASSWORD} -userName ${USERNAME} -jenaType ${JENATYPE}
done
